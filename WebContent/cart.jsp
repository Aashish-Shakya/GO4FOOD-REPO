...
<%
	...
	params.put("firstname", (String) session.getAttribute("fname"));
	params.put("email", (String) session.getAttribute("email"));
	params.put("phone", (String) session.getAttribute("mobileno"));
	params.put("productinfo", "Go4Food");
	params.put("surl", "http://localhost:8900/Go4Food/online.jsp");
	params.put("furl", "http://localhost:8900/Go4Food/fail.jsp");
					...
					if (session.getAttribute("username") != null) {
						if (map.size() > 0) {
							...
							for (Map.Entry<Dish, Integer> item : map.entrySet()) {
								totalamount += ((item.getKey().getPrice() - item.getKey().getDiscount()) * item.getValue());
								if (item.getKey() != null) {
					...
					: session.getAttribute("address").toString()%>">
							...
							<form action="<%=action1%>" method="post" name="payuForm">
								<input type="hidden" name="key" value="<%=merchant_key%>" /> <input
									...
									type="hidden" name="amount" value="<%=totalamount%>" /> <input
